// Crawling with five segments via square wave;
...
// First test: phase difference is 2*pi/5, equal to 0.2s; Stride duration can be changed via step length in x;
// 1) here is for the forward wave
ISR(TIMER1_COMPB_vect) //port12
{
  if(x1<=1)
  {
    if(0.8<=x1 && x1<=1){y1 = 249;}
    else y1=0;
    OCR1B = y1;
    x1 = x1+0.0005;           // count for 2000 times = 2s;
  }
  else x1=0;     
}
ISR(TIMER4_COMPC_vect) //port9
{
  if(x2<=1)
  {
    if(0.6<=x2 && x2<=0.8){y2 = 249;}
    else y2=0;
    OCR4C = y2;
    x2 = x2+0.0005;
  }
...
ISR(TIMER4_COMPB_vect) //port8
{
  if(x3<=1)
  {
    if(0.4<=x3 && x3<=0.6){y3 = 249;}
    else y3=0;
    OCR4B = y3;
    x3 = x3+0.0005;           
  }
...
ISR(TIMER3_COMPC_vect) //port7
{
  if(x4<=1)
  {
    if(0.2<=x4 && x4<=0.4){y4 = 249;}
    else y4=0;
    OCR3C = y4;
    x4 = x4+0.0005;
  }
...
ISR(TIMER3_COMPB_vect) //port6
{
  if(x5<=1)
  {
    if(0<=x5 && x5<=0.2){y5 = 249;}
    else y5=0;
    OCR3B = y5;
    x5 = x5+0.0005;           
  }
...
// 2) here is for the backward wave
//ISR(TIMER1_COMPB_vect) //port12
//{
//  if(x1<=1)
//  {
//    if(0<=x1 && x1<=0.2){y1 = 249;}
//    else y1=0;
//    OCR1B = y1;
//    x1 = x1+0.0002;           // count for 4000 times = 4s;
//  }
//  else x1=0;     
//}
//ISR(TIMER4_COMPC_vect) //port9
